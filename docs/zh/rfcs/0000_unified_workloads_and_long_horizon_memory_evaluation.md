...
RFC 0081 定义了更广泛的端到端评估架构，但没有确定本地样例与 registry 长程任务如何共享一套 workload contract。
如果缺少这层契约，每种任务来源都可能引入自己的 runner、命令或 report 格式。

长程任务还需要独立的成功定义。即使 agent 没有解决原生任务，这次运行仍然可以说明 PowerContext 是否采集了调查过程，
是否保留了 source provenance，是否在任务期间创建了 Memory，以及这些 Memory 能否在任务结束后被召回。任务完成情况
与 Memory 质量是两个相关但不同的结果。
...
Agent 可见的指令属于 Harbor task。本地任务把指令放在 Harbor task 目录中，registry task 则从固定版本的 dataset entry
获取指令。Workload manifest 不复制这些内容，避免产生两个事实来源。

Replay evidence 会记录最终解析出的 instruction identity 与经过脱敏的内容。评审者可以确认 agent 实际收到了什么，
同时 manifest 不会成为任务内容的另一个权威来源。
...
Memory acceptance 检查采集链产生的可观察 evidence：
...
Harbor reward、verifier result、运行时长与 model usage 保留为 label、score 或 metric。任务可以没有通过原生 verifier，
...
未来的 evaluation integration 可以消费该结果，但必须位于 workload runner 的下游。
...
现有本地样例保留其 Harbor task layout 与 task ID。Registry task 保留原生任务和 verifier。LoCoMo 仍然是固定采样，
不代表 benchmark 结论。这些 workload 只在 dataset 来源与声明的 acceptance threshold 上存在差异。

# 代价

Harbor 会成为 agent workload 执行的必要依赖。长程任务需要更多时间，并可能消耗付费模型额度。Evidence 可能包含
...
本 RFC 不替代 RFC 0081，不定义 benchmark leaderboard、通用 dataset registry 或新 agent protocol，也不替代任务原生
