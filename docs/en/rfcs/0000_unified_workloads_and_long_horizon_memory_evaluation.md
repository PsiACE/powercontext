...
Workloads share one catalog, setup contract, replay envelope, Memory evaluator, report format, and `acceptance`
command. The current implementation uses Bub as its execution adapter because Bub exposes model calls, tools, context
...
- What PowerContext state existed before execution?
...
- declared PowerContext setup in an isolated scope;
...
server and records their resolved versions in replay evidence. Harbor task definitions own task and setup timeouts;
...
workload can declare approved Experiences in `setup`. The harness creates that state through the public Client,
flushes pending setup Sources, and records a pre-execution Memory baseline.
...
baseline. It invokes the adapter, records final Memory, and runs the declared recall probes. A failed workload still
writes the evidence collected before the failure.
...
- new Memory cites setup Sources or Sources captured during execution; and
...
| Artifact | Purpose |
| --- | --- |
| `replay.json` | Workload identity, adapter, task provenance, setup, runtime observations, Memory snapshots, probes, and native evidence references. |
...
typed execution settings and native evidence while reusing workload identity, selection, setup, the replay envelope,
...
setup, evidence, evaluation, and reporting. The shared contracts keep those responsibilities in one place while the
