...
- `integrations/distribution/powercontext_integrations/assets/targets/` declares source layout and executable event bindings. Setup and package assembly share the command-manifest renderer. TypeScript adapters collect typed callbacks; the generated registrar resolves each declared handler and registers it under the declared event. Missing handlers reject registration; undeclared callbacks are not registered. WorkBuddy merges these bindings into native settings and removes stale owned events. Bub, Hermes, and framework callbacks use native class discovery, so Target does not duplicate their method lists. Operation selection, output effects, and failure handling live in executable handlers, not descriptive policy fields. `resources.json` selects compatibility tool extensions; the shared catalog drives native registration, setup grants, diagnostics, and the generated capability view. There is no separate capability manifest or source probe.
...
The catalog command reports the bindings used by actual hook registration and model tools, including compatibility extensions. It does not infer operation coverage from handler names or describe native class methods as generated hooks. Use `--list --format markdown` for the generated comparison. `make agent-resources` also refreshes the integration capability pages. MCP discovery, host permissions, and service availability remain runtime concerns; use `powercontext doctor integrations` for installed host status. OpenClaw doctor checks setup tool grants against the same catalog without overriding session policy.
